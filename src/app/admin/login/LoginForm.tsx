...
export function LoginForm({ supabaseUrl, supabaseAnonKey }: Props) {
  ...
  function getClient() {
    if (!supabaseUrl || !supabaseAnonKey) {
      throw new Error("Variáveis NEXT_PUBLIC_SUPABASE_URL e NEXT_PUBLIC_SUPABASE_ANON_KEY não configuradas no servidor.")
    }
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    let supabase
    try { supabase = getClient() } catch (err) {
      setError(err instanceof Error ? err.message : "Erro de configuração.")
      setLoading(false)
      return
    }
            ...
            {step === "mfa-enroll" && (
              <>
                ...
                <p className="text-[13px] text-muted-foreground mb-4">
                  Escaneie o QR code com Google Authenticator ou Authy e digite o código gerado.
                </p>
                {qrCode && (
                ...
                {secret && (
                  <div className="mb-4 bg-muted/50 rounded-md px-3 py-2">
                    ...
                    <p className="text-[12px] font-mono break-all">{secret}</p>
                  </div>
                )}
