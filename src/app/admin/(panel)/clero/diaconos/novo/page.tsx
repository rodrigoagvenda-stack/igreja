...
import { createDiacono } from "../actions"

export const metadata = { title: "Cadastrar Diácono" }
...
export default async function NovoDiaconoPage() {
    ...
    <div className="p-8 max-w-[700px] w-full mx-auto">
      <div className="flex items-end justify-between mb-8 pb-6 border-b border-border">
        <div>
          ...
          <h1 className="font-serif text-[28px] font-bold">Cadastrar diácono</h1>
        </div>
      </div>

      <form action={createDiacono} className="space-y-6">
        <div className="bg-card ring-1 ring-foreground/10 rounded-xl p-6 space-y-5">
          <div>
            <label className={labelCls}>Nome completo *</label>
            <input name="nome" required className={inputCls} placeholder="Dc. Nome Sobrenome" />
          </div>
