...
export default async function AdminDiaconosPage({
  ...
  const { data: diaconos, count } = await supabase
    .from("arq_diaconos")
    ...
    <div className="p-8 max-w-[1100px] w-full mx-auto">
      <div className="flex items-end justify-between mb-8 pb-6 border-b border-border">
        <div>
          ...
          <h1 className="font-serif text-[28px] font-bold">
            Diáconos Permanentes {total > 0 && <span className="text-muted-foreground text-[18px] font-normal">({total})</span>}
          </h1>
        </div>
        <Link
      ...
      {diaconos && diaconos.length > 0 ? (
        <>
              ...
              {diaconos.map((dc) => {
                ...
                return (
                  <div key={dc.id} className="grid grid-cols-[1fr_auto_auto_auto_auto] items-center px-5 py-3.5 hover:bg-muted/40 transition-colors">
                    ...
                    <Link href={`/admin/clero/diaconos/${dc.id}/editar`} className="w-8 h-8 rounded-md flex items-center justify-center text-muted-foreground hover:text-primary hover:bg-primary/10 transition-colors">
                      <IconPencil size={14} />
                    </Link>
          ...
          <Pagination page={page} total={total} pageSize={PAGE_SIZE} basePath="/admin/clero/diaconos" />
        </>
      ) : (
