    ...
    <div className="p-8 max-w-[700px] w-full mx-auto">
      <div className="flex items-end justify-between mb-8 pb-6 border-b border-border">
        <div>
          ...
          <h1 className="font-serif text-[28px] font-bold">Editar padre</h1>
        </div>
      </div>
      ...
      <form action={action} className="space-y-6">
        <div className="bg-card ring-1 ring-foreground/10 rounded-xl p-6 space-y-5">
          <div>
            ...
            <input name="nome" required defaultValue={padre.nome} className={inputCls} />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelCls}>Data de nascimento</label>
              <input name="nascimento" type="date" defaultValue={padre.nascimento ?? ''} className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Data de ordenação</label>
              <input name="ordenacao" type="date" defaultValue={padre.ordenacao ?? ''} className={inputCls} />
            </div>
          </div>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" name="ativo" value="true" defaultChecked={padre.ativo} className="w-4 h-4 accent-primary" />
