...
import { createSeminarista } from "../actions"

export const metadata = { title: "Cadastrar Seminarista" }
...
export default async function NovoSeminaryPage() {
    ...
    <div className="p-8 max-w-[700px] w-full mx-auto">
      <div className="flex items-end justify-between mb-8 pb-6 border-b border-border">
        <div>
          ...
          <h1 className="font-serif text-[28px] font-bold">Cadastrar seminarista</h1>
        </div>
      </div>
      ...
      <form action={createSeminarista} encType="multipart/form-data" className="space-y-6">
        <div className="bg-card ring-1 ring-foreground/10 rounded-xl p-6 space-y-5">
          <div>
            ...
            <input name="nome" required className={inputCls} placeholder="Nome completo" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select name="ano_formacao" required className={inputCls}>
                {anosFormacao.map(a => (
